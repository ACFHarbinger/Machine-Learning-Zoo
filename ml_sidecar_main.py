...
class MlRequestHandler:
    ...
    async def dispatch(self, method: str, params: dict, progress_callback: Callable | None = None) -> Any:
        handler = self._handlers.get(method)
        if not handler:
            raise ValueError(f"Method {method} not supported by ML sidecar")
        return await handler(params, progress_callback)

    async def _health_ping(self, p, _cb): return {"status": "ok", "sidecar": "ml"}
    ...
    async def _model_list(self, p, _cb): return {"models": self.registry.list_models()}
    ...
    async def _training_list(self, p, _cb): return {"runs": await self.training.list_runs()}
...
async def _handle_request(handler, transport, request):
    ...
    try:
        result = await handler.dispatch(request["method"], request.get("params", {}), 
