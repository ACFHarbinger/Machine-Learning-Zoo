...
class ConvBackbone(Backbone):
    ...
    def __init__(self, config: ConvBackboneConfig) -> None:
        ...
        pool_class = (
            ...
            else (nn.AvgPool1d if config.conv_type == "1d" else nn.AvgPool2d)
        )

        layers = []
        in_channels = input_dim

        for i, (kernel_size, out_channels) in enumerate(zip(config.kernel_sizes, config.channels)):
            layers.extend(
                [
                    conv_class(in_channels, out_channels, kernel_size, padding=kernel_size // 2),
