...
class TrainingProgressCallback(BaseCallback):
    ...
    def on_epoch_end(
        self, epoch: int, metrics: dict[str, float], **kwargs: Any
    ) -> None:
        self.progress_fn(
            {
                "run_id": self.run_id,
    ...
    def on_batch_end(
        self, batch_idx: int, metrics: dict[str, float], **kwargs: Any
    ) -> None:
...
class TrainingService:
    ...
    async def start(
        ...
        task = asyncio.create_task(
            self._run_training(run_id, config, progress_callback)
        )
