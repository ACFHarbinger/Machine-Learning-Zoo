...
class TrainingService:
    ...
    async def deploy(self, run_id: str, tool_name: str, device: str | None = None) -> dict[str, Any]:
        """
                ...
                deployed.append(
                    {
                        ...
                        "task_type": run.task_type,
                        "metrics": run.metrics,
                        "loaded": (self.registry.get_model(run.tool_name) is not None if self.registry else False),
                    }
                )
        return deployed
